<template>
  <main>
    <div class="container">
      <main-head />
      <!--Хлебные крошки-->
      <bread-crumbs :bread-crumbs="[{name:'Главная',ref:'#'},{name:'Дашборд',ref:null}]"/>
      <div class="title-row">
        <app-title
            size="S"
            tag="h3"
      ...
      <div class="navigation">
        <span :class="{active:navActive==0}" @click="navActive=0">Общий</span>
        <span :class="{active:navActive==1}" @click="navActive=1">Wildberries</span>
        <span :class="{active:navActive==2}" @click="navActive=2">Ozon</span>
      </div>
      <!--Фильтры-->
      <filters-block />
      <!--Переключатели-->
      <div class="row-switch">
        <app-switch :is-single="false" :label="'Учитывать СТОРНО'"  :bg="'white'"/>
        <app-switch :is-single="false" :label="'Учитывать СПП'" :bg="'white'"/>
      </div>
      <!--Периоды-->
      <div class="periods">
        <div class="left">
          ...
          <div class="row">
            <app-select :list="periods.viewingPeriod.left" :select="'4 недели'"/>
            <app-select :list="periods.viewingPeriod.right" :select="'Дек 6 - Дек 30'"/>
          </div>
        </div>
        <div class="right">
          <label>Период сравнения:</label>
          <div class="row">
            <app-select :list="periods.comparisonPeriod.left"  :select="'4 недели'"/>
            <app-select :list="periods.comparisonPeriod.right" :select="'Ноя 6 - Ноя 30'"/>
          </div>
        </div>
      </div>
      <!--Линейный график -->
      <app-chart :type="'line'" :title="'График выкупаемости'" :label="'Динамика продаж, возвратов и заказов'" />
      <div class="chart-row">
        <app-chart :type="'doughnut'" :title="'Выручка по брендам ₽'" :label="'Динамика продаж'" />
        <app-chart :type="'bar'" :title="'Выручка по магазинам ₽'" :label="'Динамика продаж'" />
      </div>
      <!--блок с таблоидами-->
      <tabloids-block />
...
import tabloidsBlock from "@/components/TabloidsBlock.vue";
import AppSwitch from "@/components/UI/app-switch.vue";
import MainHead from "@/components/MainHead.vue";
import AppSelect from "@/components/UI/app-select.vue";
import AppChart from "@/components/UI/app-chart.vue";
export default {
  data(){
     return{
       navActive:0,
  ...
  components: {AppSelect, AppSwitch , AppTitle, BreadCrumbs ,FiltersBlock , tabloidsBlock , MainHead , AppChart},
};
</script>
