<template>
  <main>
    <div class="container">
      <main-head />
      <!--Хлебные крошки-->
      <bread-crumbs :bread-crumbs="[{name:'Главная',ref:'#'},{name:'Загрузка данных',ref:null}]"/>
      <div class="title-row">
        <app-title
            size="S"
            tag="h3"
      ...
      <div class="block-setting" v-for="block in 3" :key="block">
        <app-title
            size="S"
            tag="h5"
        >
        ...
        <div class="row-button">
          <app-button  :align="'left'" :text="'Загрузить'" :type="2"/>
          <app-button  :align="'right'" :text="'Посмотреть загруженные данные'" :type="4"/>
        </div>
...
import AppFile from "@/components/UI/app-file.vue";
export default {
  data(){
     return{
       options:[
  ...
  components: {AppFile, AppSelect, AppButton, AppTitle, BreadCrumbs , MainHead},
};
</script>
