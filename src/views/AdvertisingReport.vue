  ...
  <main>
    <div class="container">
      ...
      <div class="navigation">
        <span :class="{active:navActive==0}" @click="navActive=0">Дашборд</span>
        <span :class="{active:navActive==1}" @click="navActive=1">Детальная аналитика</span>
      </div>
      <!--Фильтры-->
      <filters-block />
...
import AppClientTable from "@/components/UI/app-client-table.vue";
export default {
  data(){
     return{
       navActive:1,
  ...
  components: {AppClientTable,   AppTitle, BreadCrumbs ,FiltersBlock ,   MainHead  },
};
</script>
...
.block-info .info.active{
  border-radius: 10px;
  background: #1890FF;
  box-shadow: 0px 5px 15px 0px rgba(0, 0, 0, 0.05);
}
.block-info .info.active h3 , .block-info .info.active h4{
  color: white;
}
</style>
